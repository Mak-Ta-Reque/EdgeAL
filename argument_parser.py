...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='deeplab',
                        choices=['deeplab', 'unet', 'y_net_gen', 'y_net_gen_ffc'],
    ...
    parser.add_argument('--dataset', type=str, default='scannet-sample',
                        choices=['scannet', 'duke', 'UMN', 'AROI', "idrid-dataset", 'scenenet-rgbd', 'matterport3d','scannet-sample'],
    ...
    parser.add_argument('--loss-type', type=str, default='ce+dice',
                        choices=['ce', 'focal', 'ce+dice'],
    ...
    parser.add_argument('--resume', type=str, default=None, metavar='N',
                        help='If true start from the given path value')
    parser.add_argument('--batch-size', type=int, default=None,
                        metavar='N', help='input batch size for \
                                training (default: auto)')
    parser.add_argument('--use-balanced-weights', action='store_true', default=False, # in duke it was false
    ...
    parser.add_argument('--g-ratio', type=float, default=0.5, help=' y_net_gen_ffc hyperparameter')
    parser.add_argument('--ffc-lambda', default=0, type=float)
    args = parser.parse_args()
    args.cuda = not args.no_cuda and torch.cuda.is_available()
    print(args.cuda)
    if args.cuda:
        args.device = f'cuda:{args.gpu_ids}' #torch.device(f'cuda:{args.gpu_ids}')
    else:
